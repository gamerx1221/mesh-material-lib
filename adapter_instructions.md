...
2. Implement `ITextureAssetResolver` using the application asset system and codecs. Validate source data, preserve every representable stored mip/layer/face subresource with its row/slice pitch, normalize only where the contract requires it, and return structured diagnostics rather than silently decompressing or discarding unsupported container structure. Document extension routing and codec priority. The NVRHI adapter demonstrates this for EXR (flat scanline/tiled HALF/FLOAT R/RG/RGB/RGBA only, no deep/multipart/chromaticity conversion), JPEG XR (8-bit gray/RGB/RGBA only, no ICC/HDR/high-bit-depth/YUV conversion), and direct KTX/KTX2 (`R8`, `RG8`, `RGBA8` UNORM/sRGB only, stored topology preserved). KTX2 Zstd may be expanded only by the container library to supported direct bytes. BasisLZ/UASTC/Basis Universal is not direct decoding: reject it until a renderer-specific BasisU transcode target policy exists; direct KTX2 support does not implement glTF `KHR_texture_basisu`.
